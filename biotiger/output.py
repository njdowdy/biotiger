...
def check_opts(opts):
    ...
    f = opts.format
    if f is None or f not in ['0', '1', '2', '3', '4', '5']:
        die_with_message("Invalid -f value: %s" % f)
    if opts.bins is not None:
        try:
            int(opts.bins)
        except ValueError:
            die_with_message("Invalid -b option '%s'. Please provide an integer" % opts.bins)
...
def generate_nexus(binned_data, seq_data, format_opt):  # function still unfinished
    # def generate_nexus(binned_data, seq_data, excl_bins, mask, format_opt):  # excl_bins, mask are unused
    nexus = ''

    if format_opt == '5':  # just basic nexus file output
        species_order = seq_data.keys()
    else:
        species_order = sorted(seq_data.keys())

    nexus += nexus_header(binned_data, species_order, seq_data[species_order[0]])

    # padded_species = pad_str(seq_data.keys()) # padded_species unused
    ...
    if format_opt == '5':  # just basic nexus file output
        longest = 0
        for key, value in seq_data.iteritems():
            name_len = len(key)
            if name_len > longest:
                longest = name_len
        for key, value in seq_data.iteritems():
            prefix = '\t'
            lab = "'" + key + "'"
            spaces = ' ' * ((longest + 5) - len(key))
            charset = value
            newline = '\n'
            nexus += prefix + lab + spaces + charset + newline
        # print value
        # add nexus tail
        nexus_tail = ';\n\n\n\nEND;\n'
        nexus += nexus_tail
...
def nexus_header(binned_data, species, rep_seq):
    ...
    nex_header += "\tFORMAT DATATYPE = %s GAP = - MISSING = N;\nMATRIX\n\n" % detect_datatype(rep_seq)
...
def detect_datatype(seq):
    ...
    n_len = float(len(seq_c))
    perc = (n_len / o_len) * 100
...
def pad_str(taxon_names):
    ...
    for name in taxon_names:
        ...
        pad_str_in = ' ' * pad_len
        padded[name] = "%s%s" % (name, pad_str_in)
...
def run(opts):
    ...
    binned_data = bin_func(rates, opts.bins)
    ...
    if opts.format == '4':  # output fasta
        ...
        # output_str = generate_nexus(binned_data, seq_data, excl_bins,
        # opts.mask, opts.format)  # excl_bins, opts.mask unused
        output_str = generate_nexus(binned_data, seq_data, opts.format)
        suffix = '.nex'
